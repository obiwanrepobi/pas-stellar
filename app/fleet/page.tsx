...
import { boats, initialCleaning } from "./data";
import type { Boat, CleaningStage } from "./data";
import { reservations } from "../reservations/data";
import { FleetCtx } from "./fleetState";
import MapView from "./MapView";
import ListView from "./ListView";
import BoatModal from "./BoatModal";

const rentalBoats = boats.filter((b) => b.category !== "Utility");
// Boats scheduled out today (Jul 18) — from the reservation data.
const outToday = new Set(reservations.map((r) => r.boatId));

const STAGES: CleaningStage[] = ["needs-cleaning", "getting-cleaned", "clean"];

export default function FleetPage() {
  const [view, setView] = useState<"map" | "list">("map");
  const [selectedBoat, setSelectedBoat] = useState<Boat | null>(null);
  const [cleaning, setCleaning] = useState<Record<string, CleaningStage>>(initialCleaning);
  const [cleanStamp, setCleanStamp] = useState<Record<string, { by: string; date: string }>>({});

  const cycleClean = (id: string) => {
    const cur = cleaning[id] ?? "clean";
    const next = STAGES[(STAGES.indexOf(cur) + 1) % STAGES.length];
    setCleaning((prev) => ({ ...prev, [id]: next }));
    setCleanStamp((prev) => {
      if (next === "clean") return { ...prev, [id]: { by: "Nate", date: "Jul 18" } };
      const copy = { ...prev };
      delete copy[id];
      return copy;
    });
  ...
  const needsCleaning = rentalBoats.filter((b) => (cleaning[b.id] ?? "clean") !== "clean").length;
  ...
  const stats = [
    ...
    { label: "Needs Cleaning", value: needsCleaning, color: "#3b82f6" },
  ];

  // Morning checks only matter for boats actually going out today.
  const outTodayBoats = rentalBoats.filter((b) => outToday.has(b.id) && b.status !== "out-of-service");
  const morningPending = outTodayBoats.filter((b) => !b.morningCheckDone);
  const morningDone = outTodayBoats.length - morningPending.length;

  return (
    <FleetCtx.Provider value={{ cleaning, cleanStamp, cycleClean, outToday }}>
      <div className="px-8 py-6 max-w-[1800px] mx-auto w-full">
          ...
          {stats.map((s) => (
            <div key={s.label} className="bg-white rounded-xl p-4 shadow-[rgba(0,0,0,0.08)_0px_4px_16px]">
              ...
              <div className="flex items-end gap-2">
                ...
                {s.color && (
                  <span className="w-2.5 h-2.5 rounded-full mb-1 flex-shrink-0" style={{ backgroundColor: s.color }} />
                )}
              </div>
              {s.sub && <p className="text-[11px] text-[#afafaf] mt-1">{s.sub}</p>}
            </div>
          ))}
        </div>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <p className="text-xs text-[#afafaf]">
            Morning checks (out today): {morningDone} / {outTodayBoats.length} complete
          </p>
        </div>
        ...
        {selectedBoat && <BoatModal boat={selectedBoat} onClose={() => setSelectedBoat(null)} />}
      </div>
    </FleetCtx.Provider>
  );
}
